<script lang="ts">
	...
	import type { RoomParams, TopicType } from '$lib/interfaces';
	...
	onMount(async () => {
		...
		if (type == 'c') {
			bubble.content = await decrypt(roomParams.rK, roomParams.rP, bubble.content);
		}
	});
</script>
...
{#if bubble.tagValue('sig') == undefined}
	<div class="grid {ownId ? 'grid grid-cols-[1fr_auto]' : 'grid-cols-[auto_1fr]'} gap-1">
		{#if !ownId}<Avatar initials={roomParams.r} width="w-11" background="bg-surface-500/30" />{/if}
		<div
			class="card p-2 rounded-tl-none variant-soft-primary {ownId
				? 'variant-soft'
				: 'variant-soft-primary'}"
		>
			<header class="flex justify-between items-center flex-wrap">
				<p class="font-bold">{roomParams.r}</p>
	...
	<div class="grid grid-cols-[auto_1fr] gap-2">
		{#await fetchUserProfile(bubble.tagValue('p')) then value}
			<Avatar
				...
				width="w-11"
			/>
			<div class="card p-2 rounded-tr-none border-l-2 border-opacity-5 variant-soft-primary">
				<header class="flex justify-between items-center">
