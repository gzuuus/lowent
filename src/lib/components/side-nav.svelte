...
<div class=" grid grid-rows-[auto_1fr_auto] gap-2">
    <div class=" flex flex-col gap-2">
        <section class="flex flex-row justify-between px-2">
            ...
            <button type="button" class="btn-icon btn-icon-sm variant-filled" on:click={() => toggleSearch = !toggleSearch}>
                <SearchIcon size={16} />
            </button>
            </section>
        ...
        <section class="{toggleSearch ? 'input-group' : 'hidden'} input-group-divider grid-cols-[1fr_auto]">
            <input
                class="input"
                type="search"
