<script lang="ts">
	// Components
	import NDK, { NDKPrivateKeySigner, NDKEvent, NDKNip07Signer, type NDKUserProfile } from "@nostr-dev-kit/ndk";
	...
	import { announceTopic, fetchUserProfile, finaliceEvent, handleDeleteTopic, unixToDate, verifyAuthEvent } from "$lib/helpers";
...
<div class="grid grid-rows-[1fr_auto] h-full max-h-screen">
	...
	<header class="grid grid-cols-[1fr_auto] h-fit border-b border-surface-700 px-2 items-center">
		...
		<section class=" inline-flex items-baseline">
			{#if !roomProfile}
				<button class="btn btn-sm" on:click={handleAnnounceTopic}><span class=" inline-flex gap-1 badge variant-ghost"><GlobalIcon size={16} />Announce</span></button>
			{/if}
			<a href={roomProfile ? `${outNostrLinksUrl}/${npubEncode(rP)}` : `${outNostrLinksUrlCLient}/${npubEncode(rP)}/notes`} target="_blank" rel="noreferrer"><span class=" inline-flex gap-1 badge variant-ghost"><LinkOut size={16} />See outside</span></a>
		</section>
	</header>
